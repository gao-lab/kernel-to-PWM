...
PWM.type.and.auc.augmented.with.original.kernel.model.across.all.CMs.data.frame <- adply(cm.motif.name.and.accession.data.frame, .margins=1, .fun=function(row.values.data.frame){
    ...
    if (file.exists(paste(sep="", CM.dir.filename.character, "for.2..7.6.PWM.type.and.auc.txt")) == FALSE){
    ...
    PWM.type.and.auc.data.frame <- read.table(paste(sep="", CM.dir.filename.character, "for.2..7.6.PWM.type.and.auc.txt"), header=TRUE, sep="\t", stringsAsFactors=FALSE)

    PWM.type.and.auc.augmented.with.original.kernel.model.data.frame <- rbind(PWM.type.and.auc.data.frame, data.frame(PWM.type="original.kernel", AUC=original.kernel.training.and.validation.auc.numeric, stringsAsFactors=FALSE))
...
write.table(PWM.type.and.auc.augmented.with.original.kernel.model.across.all.CMs.data.frame, file="../data/for.2..8.AUC.summary.for.Rfam.txt", col.names=TRUE, row.names=FALSE, sep="\t", quote=FALSE)
