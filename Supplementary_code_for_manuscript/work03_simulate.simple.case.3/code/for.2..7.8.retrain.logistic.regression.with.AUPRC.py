...
PWM_type_and_auroc_and_auprc_list = []
...
for temp_PWM_type, temp_DataFrame in training_and_validation_MLL_DataFrame.groupby("PWM.type"):
    ...
    temp_auroc = sklearn.metrics.roc_auc_score(y_true=y_vector, y_score=y_predicted_vector)
    temp_auprc = sklearn.metrics.average_precision_score(y_true=y_vector, y_score=y_predicted_vector)
    PWM_type_and_auroc_and_auprc_list.append([temp_PWM_type, temp_auroc, temp_auprc])


pandas.DataFrame(PWM_type_and_auroc_and_auprc_list, columns=['PWM.type', 'AUROC', 'AUPRC']).to_csv(path_or_buf=CM_dir_str + "for.2..7.8.PWM.type.and.auroc.and.auprc.txt", sep="\t", header=True, index=False)
