...
AUC.summary.data.frame <- read.table("../data/for.2..8.AUC.summary.for.Rfam.txt", header=TRUE, sep="\t", stringsAsFactors=FALSE)
...
AUC.summary.paired.data.frame <- ddply(AUC.summary.data.frame, .variables="CM.accession", .fun=function(temp.data.frame){
    cross.comparison.data.frame <- adply(expand.grid(first.PWM.type=temp.data.frame[, "PWM.type"], second.PWM.type=temp.data.frame[, "PWM.type"], stringsAsFactors=FALSE), .margins=1, .fun=function(row.values.data.frame){
        ...
        first.PWM.type.AUC.numeric <- temp.data.frame[temp.data.frame[, "PWM.type"] == first.PWM.type.character, "AUC"]
        second.PWM.type.AUC.numeric <- temp.data.frame[temp.data.frame[, "PWM.type"] == second.PWM.type.character, "AUC"]
        return(data.frame(first.PWM.type=first.PWM.type.character, second.PWM.type=second.PWM.type.character, first.PWM.type.AUC=first.PWM.type.AUC.numeric, second.PWM.type.AUC=second.PWM.type.AUC.numeric ))
    })
...
png("../data/for.2..9.AUC.difference.scatterplot.png", width=800, height=600)
print(ggplot(AUC.summary.paired.data.frame, aes(x=first.PWM.type.AUC, y=second.PWM.type.AUC)) + geom_point() + geom_abline(slope=1, intercept=0, color="red") + facet_grid(second.PWM.type~first.PWM.type))
dev.off()

png("../data/for.2..9.AUC.difference.scatterplot.2.png", width=800, height=600)
print(ggplot(subset(AUC.summary.paired.data.frame, first.PWM.type=="Deepbind" & second.PWM.type == "exp_enlarged_by_1.0"), aes(x=first.PWM.type.AUC, y=second.PWM.type.AUC, color=original.kernel.auc)) + geom_point() + geom_abline(slope=1, intercept=0, color="red")  + xlim(0.5, 1) + ylim(0.5, 1) + scale_color_gradientn(colors=rainbow(10)[1:8], limits=c(0.5, 1)) + theme(panel.background = element_rect(fill = 'black'), text=element_text(size=28)) + labs(x = "AUC of Deepbind's transformation", y = "AUC of our transformation",  color = "AUC of the original \n model (with kernels)"))
dev.off()
