...
## 4. generate the benchmarking dataset (moved to for.2..2.3.generate.real.dataset.with.smaller.base.py)
...
if motif_params_dict["has_avg_pooling"] == 1:
    with open("../data/for.2..2.1.stop.flag.for.motif." + motif_id_str + ".txt", "w") as flag_filehandle:
        flag_filehandle.write("Is average pooling model, give up on this model.")
        flag_filehandle.write("\n")
    exit()
...
exp_PFM_ReLU_threshold_log_probability_vector = numpy.zeros(convolution_W_tensor.shape[2])
...
for temp_kernel_index in numpy.arange(0, motif_params_dict["num_detectors"]):
    ...
    temp_ReLU_threshold_log_probability = 0 - numpy.sum( numpy.log(temp_C_corrected_matrix.sum(axis=1)) / ln_of_optimal_base )
    ...
    exp_PFM_ReLU_threshold_log_probability_vector[temp_kernel_index] = temp_ReLU_threshold_log_probability
...
if set(analysis_valid_result_for_all_sequences_DataFrame["kernel.index"]) != set(numpy.arange(0, motif_params_dict["num_detectors"])):
    ## give up running this motif
    with open("../data/for.2..2.1.stop.flag.for.motif." + motif_id_str + ".txt", "w") as flag_filehandle:
        flag_filehandle.write("Kernel incomplete: lacking kernel(s)" + repr(set(numpy.arange(0, motif_params_dict["num_detectors"])) - set(analysis_valid_result_for_all_sequences_DataFrame["kernel.index"])))
        flag_filehandle.write("\n")
    exit()
...
temp_step_7_hdf5_filehandle.create_dataset(name="exp_PFM_ReLU_threshold_log_probability_vector", data=exp_PFM_ReLU_threshold_log_probability_vector)
