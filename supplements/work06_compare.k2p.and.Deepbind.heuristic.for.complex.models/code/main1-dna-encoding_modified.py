...
# load data
train_pi_labels_onehot = np.load('./train_pi_labels_onehot.out.npy')
train_reduced_annotation_labels_bow = np.load('./train_reduced_annotation_labels_bow.out.npy')
cl_weight = pickle.load(open('./class_weight.out', 'rb'))
val_pi_labels_onehot = np.load('./val_pi_labels_onehot.out.npy')
val_reduced_annotation_labels_bow = np.load('./val_reduced_annotation_labels_bow.out.npy')
train_dna_seqs = pickle.load(open('./train_dna_seqs.out', 'rb'))
test_pi_labels_onehot = np.load('./test_pi_labels_onehot.out.npy')
...
for z in range(num_chunks):
    np.save("./data"+str(z),np.transpose(convert_onehot2D(train_dna_seqs[z*chunk_size:(z+1)*chunk_size]), axes=(0,2,1)))
    np.save("./labels"+str(z),train_pi_labels_onehot[z*chunk_size:(z+1)*chunk_size])
