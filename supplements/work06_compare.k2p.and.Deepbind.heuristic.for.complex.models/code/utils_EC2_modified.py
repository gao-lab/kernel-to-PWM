    ...
    for p in json_plasmids:
        ...
        if len(p['sequences']['public_addgene_full_sequences']) > 0:
            for t in p['sequences']['public_addgene_full_sequences']:
                seqlen += len(convert_seq_to_atgcn(t["sequence"]))
        elif len(p['sequences']['public_user_full_sequences']) > 0:
            for u in p['sequences']['public_user_full_sequences']:
                seqlen += len(convert_seq_to_atgcn(u["sequence"]))
        else:
            for v in p['sequences']['public_addgene_partial_sequences']:
                seqlen += len(convert_seq_to_atgcn(v["sequence"]))
            for w in p['sequences']['public_user_partial_sequences']:
                seqlen += len(convert_seq_to_atgcn(w["sequence"]))

        if seqlen > 0:
            if len(p["pi"]) > 0: pi_name = ' & '.join([ "_" + str(temp_pi) for temp_pi in p["pi"]])
            else: pi_name = "No PI"
...
def get_seqs_annotations(json_plasmids, pi_plasmid_dict, filter_length, max_seq_length):
    ...
    for p in json_plasmids:
        ...
        if seqlen > 0:
            if len(p["pi"]) > 0:
                if ' & '.join([ "_" + str(temp_pi) for temp_pi in p["pi"]]) in remaining_pis:
                    # If PI in list above submission threshold, concatenate seqs, add annotations
                    pis[count] = (' & '.join([ "_" + str(temp_pi) for temp_pi in p["pi"]]))
                    
                    if len(p['sequences']['public_addgene_full_sequences']) > 0:
                        for t in p['sequences']['public_addgene_full_sequences']:
                            seqs[count] += ((convert_seq_to_atgcn(t["sequence"]) + 'N'*filter_length))#.encode())
                    elif len(p['sequences']['public_user_full_sequences']) > 0:
                        for u in p['sequences']['public_user_full_sequences']:
                            seqs[count] += ((convert_seq_to_atgcn(u["sequence"]) + 'N'*filter_length))#.encode())
                    else:
                        for v in p['sequences']['public_addgene_partial_sequences']:
                            seqs[count] += ((convert_seq_to_atgcn(v["sequence"]) + 'N'*filter_length))#.encode())
                        for w in p['sequences']['public_user_partial_sequences']:
                            seqs[count] += ((convert_seq_to_atgcn(w["sequence"]) + 'N'*filter_length))#.encode())
